<script lang="ts">
  // The olive brand panel shown beside every credential form (register, sign-in,
...
<div
  class="relative hidden w-5/12 shrink-0 flex-col overflow-hidden bg-brand p-10 text-brand-foreground lg:flex"
  role="group"
  aria-label="freehire"
  onmouseenter={stop}
  onmouseleave={play}
...
>
  ...
  <div class="mx-auto my-auto max-w-sm">
    ...
    <div class="relative mt-6 h-64 w-full overflow-hidden rounded-2xl bg-brand-foreground p-4 shadow-lg">
      {#if active === 0}
        <div class="flex h-full flex-col justify-center gap-3">
          {#each [100, 72, 85] as w (w)}
            <div class="flex items-center gap-3 rounded-lg bg-brand/10 p-3">
              <div class="size-8 shrink-0 rounded-md bg-brand/15"></div>
              <div class="flex-1 space-y-1.5">
                <div class="h-2 rounded-full bg-brand/50" style:width="{w}%"></div>
                <div class="h-2 w-1/3 rounded-full bg-brand/20"></div>
              </div>
            </div>
          {/each}
        </div>
        ...
        <div class="grid h-full grid-cols-3 gap-2">
          {#each [1, 2, 1] as n, ci (ci)}
            <div class="flex flex-col gap-1.5">
              <div class="h-1.5 w-3/4 rounded-full bg-brand/30"></div>
              {#each { length: n } as _, ri (ri)}
                <div class="h-8 rounded-md bg-brand/10"></div>
              {/each}
            </div>
          {/each}
        </div>
