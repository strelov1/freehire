...
- `billing-sync` — reconciles Pro subscriptions against RevenueCat: applies `billing_events` the webhook recorded but could not apply, then re-reads subscriber state for accounts whose `pro_until` is within a day of lapsing. Hourly, and **not optional** — the provider retries a webhook five times over ~2.5h and then stops for good, so past that window this worker is the only path by which a paid subscription becomes Pro. Needs `DATABASE_URL` and the `REVENUECAT_*` credentials; without the credentials it is a **no-op that never opens the pool**. `BILLING_SYNC_MAX_PER_RUN` (default 500) bounds one run. Both passes reach candidates through `billing_events`, never `users` — the provider's subscriber GET CREATES whatever identifier it is handed, so asking about an account that never transacted would manufacture a subscriber. See [internal/identity/billing/AGENTS.md](internal/identity/billing/AGENTS.md).
