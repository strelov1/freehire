...
func (s *dbStore) CompleteOpen(ctx context.Context, entries []embed.Claimed, model string, vectors map[int64][][]float32) error {
	...
	return s.tx(ctx, func(qtx *db.Queries) error {
		...
		// Persist each job's chunk vectors in the same transaction as the stamp, so a
		// job is never marked embedded without its vectors reaching Postgres. A missing
		// entry (no chunks — an empty description, see search.jobPassages) is skipped
		// (leaves the column untouched) rather than nulling it.
		for _, e := range entries {
			...
			if err := qtx.SetSemanticEmbedding(ctx, db.SetSemanticEmbeddingParams{
				ID: e.JobID, Embedding: search.SemanticChunksToArray(v),
			}); err != nil {
