...
type PruneCandidatesParams struct {
...
func (q *Queries) PruneCandidates(ctx context.Context, arg PruneCandidatesParams) ([]PruneCandidatesRow, error) {
	...
	defer rows.Close()
	items := []PruneCandidatesRow{}
	for rows.Next() {
		var i PruneCandidatesRow
		if err := rows.Scan(
			&i.ID,
			&i.Source,
			&i.ExternalID,
...
grp AS (
    ...
    WHERE i + 1 <= cardinality(a)
      ...
      AND a[i]     <> ALL(COALESCE($2::text[], '{}'))
      AND a[i + 1] <> ALL(COALESCE($2::text[], '{}'))
      AND a[i]     <> ALL(COALESCE($3::text[], '{}'))
      AND a[i + 1] <> ALL(COALESCE($3::text[], '{}'))
    UNION ALL
    ...
    WHERE i + 2 <= cardinality(a)
      ...
      AND a[i]     <> ALL(COALESCE($2::text[], '{}'))
      AND a[i + 2] <> ALL(COALESCE($2::text[], '{}'))
      AND a[i]     <> ALL(COALESCE($3::text[], '{}'))
      AND a[i + 2] <> ALL(COALESCE($3::text[], '{}'))
      AND a[i + 1]  = ANY(COALESCE($3::text[], '{}'))
)
SELECT grp,
       count(*)                           AS jobs,
