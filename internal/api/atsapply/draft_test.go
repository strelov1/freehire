...
func TestResolveWithDrafting_DraftsAnOtherwiseUnmappedFreeTextField(t *testing.T) {
	...
	plan, err := ResolveWithDrafting(context.Background(), fields, map[string]string{}, drafter, GroundingContext{}, false, nil, 0, 0)
	if err != nil {
...
func TestResolveWithDrafting_ADeterministicallyResolvedFieldIsNeverSentToTheDrafter(t *testing.T) {
	...
	plan, err := ResolveWithDrafting(context.Background(), fields, map[string]string{"first_name": "Ada"}, drafter, GroundingContext{}, false, nil, 0, 0)
	if err != nil {
		t.Fatalf("ResolveWithDrafting: %v", err)
	}
	if len(drafter.calls) != 0 {
...
func TestResolveWithDrafting_ANilDrafterLeavesTheDeterministicPlanUnchanged(t *testing.T) {
	...
	plan, err := ResolveWithDrafting(context.Background(), fields, map[string]string{}, nil, GroundingContext{}, false, nil, 0, 0)
	if err != nil {
		t.Fatalf("ResolveWithDrafting: %v", err)
	}
	if len(plan.Unmapped) != 1 {
...
func TestResolveWithDrafting_ADrafterErrorPropagates(t *testing.T) {
	...
	if _, err := ResolveWithDrafting(context.Background(), fields, map[string]string{}, drafter, GroundingContext{}, false, nil, 0, 0); err == nil {
...
func TestResolveWithDrafting_AGeographyFieldAlreadyMappedByIDIsStillFilled(t *testing.T) {
	...
	plan, err := ResolveWithDrafting(context.Background(), fields, map[string]string{"location": "Florianópolis, Brazil"}, drafter, GroundingContext{}, false, nil, 0, 0)
	if err != nil {
		t.Fatalf("ResolveWithDrafting: %v", err)
	}
	if len(drafter.calls) != 0 {
