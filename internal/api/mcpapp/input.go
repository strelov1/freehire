...
func (in SearchInput) QueryValues() url.Values {
	...
	setList(v, "work_mode", in.WorkMode)
	setList(v, "seniority", in.Seniority)
	setList(v, "category", in.Category)
	setList(v, "skills", in.Skills)
	setList(v, "employment_type", in.EmploymentType)
	setList(v, "english_level", in.EnglishLevel)
