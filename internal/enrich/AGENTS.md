...
- `Validate` as a guard: a transport error retries once in-process; an unparseable or invalid payload is NOT retried in-process (an identical re-call would reproduce it) — the outbox attempts counter retries it on later cron runs and dead-letters only after `MaxAttempts` failures (default 3). Immediate dead-letter is reserved for corrupted rows, which can never load.
