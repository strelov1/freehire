...
- **`stage` is a controlled vocabulary** (`userjob.Stages`/`ValidStage` in `internal/userjob/stages.go`): preparing/applied/screening/responded/interview/offer/accepted/rejected/withdrawn/expired. The SPA mirrors it; an unknown stage is a 400 before any DB touch.
...
`counts.go` folds per-stage rows into the pipeline snapshot (`CountByStage`). `applied_on.go` is not stage-keyed: it bounds a day someone states they applied on (`ValidateAppliedOn`, `MaxAppliedOnAgeDays`, all behind the `ErrAppliedOnOutOfRange` sentinel the handler maps to 400) and places the day at the hour applications are stored at (`AppliedOnInstant` — noon UTC, so the stored instant renders as the day typed for readers west of Greenwich). Two surfaces take such a date — the tracker's dated apply and the ghost report — so the window lives here rather than in either caller. The counterpart distinction sits in `internal/jobtracking`: `MarkAppliedAt` (mail's upper bound — keeps an existing date) versus `MarkAppliedOn` (the candidate's stated day — overwrites).
...
**Silence thresholds carry their provenance at the point of definition.** Six specific numbers read as measurement whether or not they are one, and only two of these are: `applied` 21 is measured over 92 observed applications, `interview` 12 over six, `screening` 18 and `responded` 15 are interpolation stepping evenly between those anchors, `offer` 5 is judgement — no application in the sample has ever reached that stage — and `preparing` 21 is neither: it copies `applied`'s value to satisfy `TestSilenceThresholdsCoverExactlyTheActiveStages`, but is unreachable in practice (`jobtracking.TrackedJob.Silence()` short-circuits on `AppliedAt == nil`, guaranteed for this stage). Keep the per-value comments when tuning; a bare table invites the next reader to trust all six equally.
