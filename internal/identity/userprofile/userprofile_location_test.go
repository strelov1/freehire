...
func TestSave_StoresCombinedLocationPreferences(t *testing.T) {
	...
	if _, err := svc.Save(context.Background(), 7, []string{"backend"}, []string{"go"}, nil, nil, loc); err != nil {
		t.Fatalf("Save: %v", err)
	}
...
func TestSave_NilLocationStoresNull(t *testing.T) {
	repo := &fakeRepo{}
	if _, err := userprofile.New(repo).Save(context.Background(), 7, []string{"backend"}, []string{"go"}, nil, nil, nil); err != nil {
		t.Fatalf("Save: %v", err)
	}
...
func TestSave_EmptyLocationCollapsesToNull(t *testing.T) {
	...
	if _, err := userprofile.New(repo).Save(context.Background(), 7, []string{"backend"}, []string{"go"}, nil, nil, empty); err != nil {
		t.Fatalf("Save: %v", err)
	}
	if repo.upserted.LocationPreferences != nil {
		t.Errorf("LocationPreferences = %s, want nil", repo.upserted.LocationPreferences)
...
func TestSave_RejectsInvalidLocationValues(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			repo := &fakeRepo{}
			_, err := userprofile.New(repo).Save(context.Background(), 7, []string{"backend"}, []string{"go"}, nil, nil, tc.loc)
...
func TestSave_NormalizesLocationEnumCase(t *testing.T) {
	...
	if _, err := userprofile.New(repo).Save(context.Background(), 7, []string{"backend"}, []string{"go"}, nil, nil, loc); err != nil {
		t.Fatalf("Save: %v", err)
	}
	got := decodeLoc(t, repo.upserted.LocationPreferences)
