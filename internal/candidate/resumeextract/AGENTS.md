...
- **A redaction placeholder is blanked, in code, before anything is persisted** (`scrub.go`, between the unmarshal and the contact overwrite). The model reads a de-identified CV, so `[REDACTED_NAME_1]` in `summary` or `[REDACTED_LINK_3]` in `projects[].link` is a mask it copied, never a fact — and `resume_structured` is seeded into the base CV (`cv.Seed`) and rendered into the PDF, so one that survives reaches a recruiter. This was a line of the system prompt only, while a line two below it tells the model to copy each highlight verbatim. **Never `Redactor.Restore` here**: restoring would put the real name back into a field an anonymous page is about to serve. The walk is by reflection, so a field added to `Structured` is covered without anybody remembering; `pii.PlaceholderPrefix` is the one spelling of the marker.
