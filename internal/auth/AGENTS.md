...
4. **Middleware** (`middleware.go`): `RequireAuth(iss, versions)` reads the JWT cookie, confirms the token's generation against the database, and puts `user_id` in `c.Locals`; `RequireAuthOrKey(iss, versions, keys)` tries the cookie first, then a **full-scope** API key. `RequireAuthOrScopedKey(..., allowed...)` additionally admits the listed narrow scopes — used by the CV surface, `/auth/me`, and the profile **read** (`GET /me/profile`, so an agent can ground itself in the user's stated preferences; the profile writes stay cookie-only). A live key on a route its scope does not cover is **403** (insufficient scope), not 401.
