// compact: a single-column CV tuned for density — less vertical space between blocks and
// tighter list spacing than classic-ats, so a longer career history still fits on one page.
// Same base type size/leading/margins and the same field set as classic-ats; only the spacing
// constants differ. Uses Libertinus Serif (embedded in the typst binary) so the text layer
...
// Per-side page margins (inches) from the document; any missing or non-positive side falls
// back to 0.5in, same as every other template — the density this template adds comes from
// tighter inter-block spacing below, not from shrinking the page margin or the base type size
// (the live HTML preview assumes every template's unset style resolves to the same 9.5pt /
// 0.5em leading; diverging here would desync its pagination from the PDF's).
#let mg(k) = {
...
// falls back to classic-ats's own base, so a CV that sets nothing renders at the same type
// size and leading — see the margin note above for why. These helpers are duplicated per
// template on purpose: the renderer stages only template.typ, so an #import of a shared
...
#let rule = { v(0.08em); line(length: 100%, stroke: 0.5pt + rgb("#b3b3b3")); v(0.04em) }
...
#{
  ...
  [#text(weight: "bold", size: (11.5 / 9) * 1em)[#s(hd, "full_name")]]
...
#let exp = arr(cv, "experience")
  ...
  for e in exp {
    ...
    if dr != "" { head = head + " (" + dr + ")" }
    block(above: 0.4em, below: 0.25em)[#text(weight: "bold")[#head]]
    // Context line.
    let sum = s(e, "summary")
...
#let edu = arr(cv, "education")
  ...
  block(above: 0.3em, below: 0.2em)[#sectionLabel("Education")#h(1em)#entries.join("; ")]
}

// ---- Skills (inline, flattened) ----
#let allSkills = arr(cv, "skills").map(g => arr(g, "items")).flatten()
#if allSkills.len() > 0 {
  block(above: 0.25em, below: 0.2em)[#text(weight: "bold")[SKILLS:] #allSkills.join(", ")]
}
...
#let langs = arr(cv, "languages")
  ...
  if names.len() > 0 {
    set par(justify: false)
    block(above: 0.2em)[#text(weight: "bold")[LANGUAGES:] #names.join(", ")]
  }
}
...
#let certs = arr(cv, "certifications")
  ...
  if items.len() > 0 {
    set par(justify: false)
    block(above: 0.2em)[#text(weight: "bold")[CERTIFICATIONS:] #items.join("; ")]
  }
}
