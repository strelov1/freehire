// headshot: a single-column CV in a sans face with a portrait in the header — the layout
// most European application forms expect. Name, summary, and contacts sit left of the
// photo; everything below is the same one-column flow as modern-sans, so the reading order
// is linear. NOT ATS-safe purely because of the portrait: the text layer stays fully
// extractable and the sections keep standard headings. Uses Liberation Sans, staged by the
// renderer via --font-path.
//
// The renderer stages photo.jpg beside data.json ONLY when the member has a headshot, and
// says so via has_photo — image() on a missing file is a compile error, so the flag is what
// makes the placeholder path safe. Helpers are duplicated per template because the sandbox
// stages no shared module for #import to resolve.
...
// ---- Header: name and contacts left, portrait right ----
#let hd = cv.header
#let contacts = {
  let parts = ()
