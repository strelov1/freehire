-- Reverse of migrations/0132_pro_until_sources.sql. See the add-store-purchases change.
--
...
--   psql "$DATABASE_URL" -f deploy/rollback/0132_pro_until_sources.down.sql
--
...
-- is not reconstructible afterwards, and re-applying 0132 over the wreckage re-splits by
...
-- ROLLING FORWARD AGAIN IS NOT "RE-RUN 0132". 0132 would fail on its ADD COLUMN — the
