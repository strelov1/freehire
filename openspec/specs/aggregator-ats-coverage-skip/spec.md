# aggregator-ats-coverage-skip Specification

## Purpose

Ingest does not store an aggregator's copy of a posting when the catalogue already carries that
employer from a source it crawls directly. The saving is real — the gate discards on the order
of 1,500 of every 2,000 postings a Himalayas run offers — but so is the cost of being wrong,
and the two are not symmetric: a posting wrongly discarded is never written, so it is never in
the database, in `/find`, or in search, and leaves nothing anyone can query. A posting wrongly
kept is a duplicate the `aggregator-ats-dedup` pass marks on its own schedule. Every rule below
