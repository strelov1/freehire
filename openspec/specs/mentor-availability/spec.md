# mentor-availability Specification

## Purpose

What a mentor is free for, and the rules that turn that into bookable hours. Availability
is a recurring week plus dated overrides, stored as times carrying no zone; the mentor's
own IANA zone resolves them, and that ordering is the only one that survives daylight
saving. Slots are computed on read and never stored, expressed in the VIEWER's zone, and
bounded by buffers, minimum notice and the booking horizon.
