# role-category-alias-coverage Specification

## Purpose

The coverage contract for the classify title-alias dictionary within categories that
already exist: which named IT sub-role clusters (agile/PM, security, data/ML/DevOps) it
must resolve, the alias-ordering doctrine that keeps a specific alias from being stolen
by a generic terminal fall-through, and the word-trap exclusions (bare "safe", bare
"compliance") kept out deliberately.
