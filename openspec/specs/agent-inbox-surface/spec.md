# agent-inbox-surface Specification

## Purpose

Let a user whose own agent harness already has a mail client and an LLM run the
inbox themselves, so the feature costs freehire nothing to give them. The two
expensive halves of the hosted inbox — the transport (a verified Gmail scope, or
a mailbox SES receives for) and the classification (inference we pay for) — are
exactly what such a user already owns. This capability is the way in: the caller
