# header-filter-trigger Specification

## Purpose

The **All filters** modal trigger for jobs- and companies-backed list pages is hosted in
the shared header search box, at its right edge, mirroring the location scope-prefix on
the left. It shows an active-filter badge and opens the active page's own filter modal on
every viewport, so filters stay reachable at any scroll position without a separate
toolbar or floating affordance.
