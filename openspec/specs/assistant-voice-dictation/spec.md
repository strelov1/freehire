# assistant-voice-dictation Specification

## Purpose

Recording the caller's voice into the assistant composer, and the transcription endpoint
behind it. The experience interview asks people to narrate a career, which is what a
keyboard is worst at. Two rules run through everything here: the caller sends the words
themselves, because a transcription is a guess; and the spend is bounded without relying
on metering that does not exist yet.
