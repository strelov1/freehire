# marketing-role-taxonomy Specification

## Purpose

The catalogue's coverage contract for marketing disciplines: which clusters the role
dictionary must keep separable, which tooling the skill dictionary must resolve, and
how the `GTM` and `GEO` homonyms are partitioned across the classify/roletag/skilltag
layers so neither meaning leaks into the other.
