# interview-schedule Specification

## Purpose
Capturing when an interview is, and attaching it to the application it belongs to: the
calendar grant, what may and may not be stored, and the single identifier allowed to link
a meeting without asking. Introduced by the interviews-from-google-calendar change.
...
A meeting SHALL be linked to an application only where its `iCalUID` equals the `UID` of
the `text/calendar` part of an email that is already linked to that application. A meeting
no invitation names SHALL NOT be stored at all.

There is deliberately no weaker tier. One existed and was removed rather than repaired: it
matched an employer's name in the event title against `applications.company_slug`, which
is hyphenated — no calendar entry says "acme-inc" — so it never fired for a multi-word
employer; for a single-token one it was an unanchored substring, which reads "Q3 ramp-up
planning" as an interview with Ramp; and nothing could confirm or dismiss what it
produced, so a wrong guess was permanent. A tier that rarely fires, misfires when it does,
and cannot be corrected is worse than no tier.

A weaker tier MAY be added later, and SHALL then arrive with a way for the candidate to
confirm or dismiss what it produces, and SHALL NOT write into `application_events` — a row
there carrying an application id is a link, into an append-only table.
...
#### Scenario: A matching company name is not enough

- **WHEN** an event titled with a tracked employer's name carries no UID we hold
- **THEN** it is not linked and not stored
