# job-freshness-badges Specification

## Purpose

The two badges that tell a reader a posting is worth being early to — `New` and
`Be an early applicant` — their thresholds, the gates that stop them lying, and
the surfaces that render them.

Both read as encouragement, so both are held to a higher bar than "the number
looks small". The posting date alone does not earn either: sources rewrite it.
On production, six of the eight most-viewed postings carry a `posted_at` of
"today" against a real age of 54 to 79 days, and the reality gate is what keeps
`New` off all six.
