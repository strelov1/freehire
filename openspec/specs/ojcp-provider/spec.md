# ojcp-provider Specification

## Purpose

freehire as a conforming provider of the Open Job Context Protocol (OJCP), on the READ
side: the manifest an agent discovers us by, the three read tools, the projection from this
catalogue's own shapes into the standard's, and the rule that both transports answer
identically.

Agent-initiated application — `begin_application`, `submit_application`,
`check_application_status` — is deliberately NOT here. It touches the auto-apply queue and
belongs to its own change.

Conventions and the traps behind them live in
[internal/api/ojcp/AGENTS.md](../../../internal/api/ojcp/AGENTS.md).
