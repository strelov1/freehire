# assistant-transcript-scrolling Specification

## Purpose

When the chat transcript follows the streaming answer and when it yields to the reader.
An answer long enough to be worth re-reading is exactly the answer that used to be
impossible to re-read: the pane scrolled itself to the newest line on every arriving
frame. Following is now the reader's to keep or to leave.
