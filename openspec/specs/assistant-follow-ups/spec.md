# assistant-follow-ups Specification

## Purpose

Suggesting the next question once a turn has settled. The strip is decoration: it is
generated outside the turn loop, every failure is silence rather than an error, and it
renders as inert text because activating one speaks the model's words in the caller's
own voice.
