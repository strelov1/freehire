# skill-glossary Specification

## Purpose

Every canonical skill in `internal/dict/skilltag` carries a plain-language description of
what the thing IS — the vocabulary's third side, beside the slug the parser resolves to
and the label a reader sees. It is revealed on the skill chip and served as a glossary
under `/skills`.
...
### Requirement: Every canonical skill is described, and the build enforces it

A canonical skill with no description SHALL fail the build, the same absolute rule the
display labels carry. Adding a skill to the alias tables therefore means adding its
description in the same change.

This was reached behind a ratchet — a recorded floor that only ever rose — because the
whole vocabulary reviewed in one pull request is a review nobody performs. The floor was
deleted when it met the vocabulary: two rules for one question can disagree, and one
cannot.

#### Scenario: A skill is added without a description

- **WHEN** a canonical is added to the alias tables and no description is written for it
- **THEN** the package's tests fail, naming the skill

#### Scenario: A description is deleted

- **WHEN** an edit removes a description from a canonical still in the vocabulary
- **THEN** the package's tests fail
...
Nothing eager needs to record WHICH skills are described. While coverage was partial a
slug list shipped in the shared module so a chip could decide whether to draw its reveal
before the prose arrived; with coverage total, "described" and "canonical" name the same
set and the shared module already carries it.
...
The reveal is unconditional: every canonical carries an entry, so a chip has one to
show. `Description` still answers `""` for a slug that is not a canonical — a stale
value must not print a placeholder — but no canonical reaches that path.
...
### Requirement: Every skill has a glossary page
