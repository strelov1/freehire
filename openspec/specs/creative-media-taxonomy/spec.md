# creative-media-taxonomy Specification

## Purpose

Media production as a category of its own: video, animation, art, audio and
photography. This capability owns which titles resolve to `creative`, why it is
technical, the named roles the media crafts expose — plus the game-development
roles that deliberately keep their existing categories — and the curated
creative-toolchain skill vocabulary. It exists because those titles resolved to
NO category and NO role at all (~830 open postings), while audio sat in
`design` beside UX work purely because "Sound Designer" contains the word
"designer".

Its defining constraint is placement. The title table resolves in declaration
order, and every media craft is also a tool or a second hat inside someone
else's title, so the aliases are declared LAST — declared any higher they do
not add a facet, they take rows from `design` and `marketing`.
