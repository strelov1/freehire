# browser-tool-form-fill Specification

## Purpose

The `fill_simple` primitive this extension answers over the browser-tool wire: how a fill
addresses one control, what it does when it cannot, and the outcome vocabulary a harness
reads back.

The relay and the harness that calls it live in hire; this capability is the extension's
end of that wire, plus the one rule the Go side must hold up (naming the scope it read a
field from). Sibling to `browser-tool-page-read`, which covers `read_page`.
