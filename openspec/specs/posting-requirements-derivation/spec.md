# posting-requirements-derivation Specification

## Purpose

Reading a posting's own stated requirements out of its description markup, with no
model call: what counts as a requirements section (three closed heading
vocabularies, and why structure alone cannot decide), how the list under it becomes
`{text, priority}` entries, how those are stored per job, and how the catalogue that
predates the column is filled.

It exists because the enrichment model reaches a small share of the catalogue and is
expensive, while a large share of postings state their requirements as a list under a
heading — a shape a parser can read for free. The two sources barely overlap: the
model reads the prose postings, this reads the marked-up ones.

The output is structure, not display. It is served under `enrichment.requirements`
for a matcher or an API consumer to read; **the job page deliberately does not render
it**, because the extractor fires exactly when the description already shows that
list. See `internal/job/reqextract/AGENTS.md`.
