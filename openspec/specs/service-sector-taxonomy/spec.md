# service-sector-taxonomy Specification

## Purpose

The last clusters in the catalogue that had a shape: logistics, education,
personal services and office administration, plus the Russian vocabularies
inside them.

Its defining hazard runs opposite to the waves before it. Those guarded against
a bare alias sweeping a qualified title; here a short alias hides INSIDE a
longer word — `Делопроизводитель`, an office clerk, ends in `водитель`,
"driver". `wordmatch` matches on word boundaries and cannot make that mistake,
but nothing in an alias list shows the hazard, so the pair carries a regression
test that asserts the specific category rather than merely a non-empty one.

What remains after this capability is 684 446 open postings across 23 389 title
groups, and most of it is not classifiable by title at all: "Специалист",
"Мастер", "SUPERVISOR", "Team Leader" — plus postings that are not vacancies,
like "General Application" and "Initiativbewerbung".
