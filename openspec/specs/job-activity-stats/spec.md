# job-activity-stats Specification

## Purpose

Surface the catalogue's flow over time — how many vacancies are added versus
removed per period — as a materialized daily rollup, a public read API over it,
and a public dashboard page that visualises it.
...
recompute the rollup from `jobs` in full — as an atomic delete-and-reinsert in a
single transaction — so that the rollup is a pure function of the current `jobs`
state and re-running it is idempotent.
