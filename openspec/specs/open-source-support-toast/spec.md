# open-source-support-toast Specification

## Purpose

Ask the visitor to star the freehire repository on GitHub — the one form of support
that costs them nothing — without ever competing for attention with another ask.
Owns when the invitation may appear, how it queues behind the Product Hunt launch
strip, which surfaces it yields the bottom-right corner to, and when it stops being
shown for good.
