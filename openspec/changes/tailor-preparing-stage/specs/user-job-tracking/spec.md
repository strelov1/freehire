...
`stage = 'applied'` when the stage is currently unset, and SHALL promote it to `applied` when
the stage is currently `preparing` (an already-set stage other than `preparing` is left
untouched). Authentication MAY be by session cookie or by API key; either identifies the acting
user identically. Marking applied sets `applied_at`; it works whether or not a view was recorded
...
#### Scenario: Applying promotes a preparing stage

- **WHEN** an authenticated user applies to a job whose `stage` is currently `preparing` (set by
  starting CV tailoring, never by the candidate)
- **THEN** the interaction's `stage` becomes `applied`
- **AND** `applied_at` is set exactly as it is for a job with no prior stage

#### Scenario: Applying does not regress an advanced stage

- **WHEN** an authenticated user applies to a job whose `stage` is already `interview` or later
- **THEN** the interaction's `stage` is left unchanged at its advanced value
