...
- [ ] 1.7 Implement `experience.Import(userID, structured, stamp)`: employments matched case-insensitively on (company, role) filling only empty fields; atoms matched on a normalized claim; provenance `cv_import`, `source_ref` = upload stamp; never deletes
- [ ] 1.8 Test import reconciliation: a trimmed re-upload does not shrink the bank; a repeat upload creates no duplicates; a user-edited employment field survives re-import; a late extraction for a superseded CV still lands
...
- [ ] 2.1 Implement `experience.Retrieve(userID, query, skills)` — deterministic scoring on canonical skill intersection (dominant), claim/context token overlap, employment recency, and an `agent_inferred` penalty, returning a bounded top-N
- [ ] 2.2 Test retrieval: skill-carrying evidence outranks incidental text overlap; a large bank returns a capped result; no LLM call is made
