...
The link carries `base64url(userID "." group "." HMAC-SHA256(userID "|" group))`,
signed with the JWT secret under a fixed, distinct salt (`"email-prefs-v1"`) and
compared in constant time. Nothing is stored and nothing expires.
