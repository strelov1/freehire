...
- [x] 3.3 Implement `cmd/backfill-company-info-wikipedia`: dry-run (report proposed writes) by default, `--apply` to write; `WIKIPEDIA_BACKFILL_MAX_PER_RUN` env cap via `worker.EnvInt64`.
- [x] 3.4 Wire the worker through `internal/platform/worker` bootstrap (`DATABASE_URL` only, no other required env).
