...
**Reason**: The brand fill now belongs to `Tailor my CV` in every state, so auto-apply can no
longer be the page's primary CTA. What survives of this requirement — that clickable
auto-apply takes the phone's bar, carries the `Pro` marker, and is not pre-empted client-side
— is restated by the added requirement "Auto-apply is the offered way to apply when it can be
started".

**Migration**: None for callers. `JobCtaPlan.autoApply.primary` is replaced by
`JobCtaPlan.autoApply.leads`, which answers "is auto-apply the offered way to apply" rather
than "does it carry the brand fill"; `JobCtaPlan.external.primary` is removed outright, the
external link being outline in every state.
