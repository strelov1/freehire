...
**Reason**: Matching moves from the browser to the suggestions index. The client
shipped 1,830 role labels and 1,532 aliases and ran an edit-distance pass over
them per keystroke; Meilisearch does prefix matching, typo tolerance and ranking
over the same vocabulary plus mined titles, skills and companies.

**Migration**: The behaviour is served by `GET /api/v1/suggest`, whose dictionary
the builder fills from the same `ROLE_LABELS` and role-alias vocabularies.
`web/src/lib/roleSuggest.ts` and its tests are deleted.

### Requirement: Suggestions are ranked by match quality before open vacancies

**Reason**: The tier ladder existed to stop vacancy count from promoting a role
the query did not name, and the typo tier was dropped outright for the same
reason. The index ranks by relevance to the query, so the failure it guarded
against — Marketing Specialist above Backend Engineer for `backedn` — no longer
arises from ranking, and a typo-only match can be offered rather than discarded.

**Migration**: Ranking is specified in the `search-suggestions` capability:
relevance, then recorded search demand, then open-posting count, then shorter
text. The `backedn` case is kept as an explicit scenario there.

### Requirement: One row per role, never one per grade

**Reason**: Still true, but it is no longer a property of a client-side matcher.
