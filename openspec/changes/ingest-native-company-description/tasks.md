...
- [x] 3.1 After a board's `Fetch` succeeds, type-assert the adapter for `CompanyDescriber` and call it once; on error, log and continue (a failed company-description fetch SHALL NOT fail the board's job ingest).
