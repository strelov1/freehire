...
- [ ] 3.1 Define the wire shape and the sanitize pass in `internal/candidate/coverletter/coverletter.go`: the letter body, the cited atom ids, the language, and the server-owned bounds (body ceiling, body floor, maximum cited atoms). Test that a model body over the ceiling is clipped and that an out-of-vocabulary field is coerced, not persisted.
...
- [ ] 3.3 Implement the publishable-provenance filter over candidate atoms as a pure function, before any chain code exists. Test that `agent_inferred`, absent, and unrecognised provenance are all withheld, and that `manual` / `cv_import` / `stated_in_chat` pass.
