...
- [ ] 1.1 Add `CloseUnseenJobsForBoard :one` to `internal/platform/db/queries/jobs.sql`, beside
...
- [ ] 1.2 `make sqlc`; commit the regenerated `internal/platform/db` in the same commit.
- [ ] 1.3 Integration test in `internal/platform/db`: the board's stale open rows close; a
...
- [ ] 2.1 In `internal/ingest/pipeline`, record per board whether the run PROVED it covered that
      board: crawl did not fail, and `Ingested + Rejected + ATSCovered + Skipped > 0`. The
      second is the same disjunction `pipeline.go` already uses to decide a mid-crawl error was
      still progress — reuse it rather than restating it, so the two cannot drift.
- [ ] 2.2 Carry the qualifying boards out on the per-provider `Stats` the Runner already
...
- [ ] 2.3 A boardless entry (`board == ""`) never qualifies — `BoardPattern("")` matches the
...
- [ ] 3.1 In `cmd/ingest`, after the existing company-scoped close, run a board-scoped close per
...
- [ ] 3.2 Skip the board scope for a provider that declares `sweepGrace`, is self-closing, or is
...
- [ ] 3.3 Log each board-scoped close with its board and count. A provider-level number cannot
...
- [ ] 3.4 A per-board failure is logged and the sweep continues to the next board; the run still
...
- [ ] 4.1 A board that yielded zero postings closes nothing, though its crawl succeeded.
- [ ] 4.2 A board whose crawl failed closes nothing.
- [ ] 4.3 A boardless entry closes nothing through the board scope.
- [ ] 4.4 A provider declaring `sweepGrace` closes nothing through the board scope.
- [ ] 4.5 A board the run never reached closes nothing.
- [ ] 4.6 **The leak, closed:** a company the run wrote NO posting for, whose stale job sits on a
      board that yielded, IS closed.
- [ ] 4.7 A board that yielded only REJECTED postings still qualifies — the crawl reached them,
      and refusing would spare exactly the non-tech-heavy boards where stale rows accumulate.
- [ ] 4.8 `go build ./... && go vet ./... && go test ./...`, then `go vet -tags=integration ./...`
      and the tagged suites for `internal/platform/db`, `cmd/ingest`, `internal/ingest/pipeline`.

## 5. Documentation

- [ ] 5.1 `docs/agents/job-lifecycle.md`: mechanism (1) gains the board scope. Rewrite the
...
- [ ] 5.2 `internal/ingest/pipeline/AGENTS.md`: the board-qualification rule and why zero-yield
      is refused (the Workday `total:0` truncation).
- [ ] 5.3 `cmd/liveness`'s `probeDespiteRegistered` comment cites the company-scope leak as its
