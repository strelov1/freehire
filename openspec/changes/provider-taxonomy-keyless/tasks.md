...
- [ ] 1.1 RED: in `usajobs_test.go`, `reed_test.go` and `whatjobs_test.go`, restate each
...
- [ ] 1.2 RED: add a test that the taxonomy registry is total — `AggregatorProviders(All(nil))`
...
- [ ] 1.3 GREEN: in `registry.go`, register the three on the `c == nil` path with an empty
...
- [ ] 1.4 Add `sources.Taxonomy()` — a wrapper over `All(nil)` carrying the "no transport, never
      Fetch here; crawlers call All" rule — and point `FilterableProviders` at it.
- [ ] 1.5 Confirm `filterableProviders` and `SweepGraceWindows` still behave: `whatjobs` keeps
...
- [ ] 2.1 `cmd/reindex/main.go` — call `sources.Taxonomy()`, and delete the stale rationale
...
- [ ] 2.2 `cmd/ghost-crosscheck/main.go:83-95` — call `sources.Taxonomy()`; check the empty-list
      guard and the comment above it still read true.
- [ ] 2.3 `internal/handler/status.go:123` — call `sources.Taxonomy()`, dropping the "nil client
...
- [ ] 2.4 Grep for any remaining `sources.All(nil)` outside the sources package and convert it,
      so the two entry points cannot drift back together.

## 3. Frontend contract and labels

- [ ] 3.1 Run `make gen-contracts` with the three variables unset and verify `SOURCE_VALUES`
      gains exactly `reed`, `usajobs` and `whatjobs` and nothing else changes in the file.
- [ ] 3.2 Adjacent fix, not caused by S7: add `usajobs: 'USAJobs'` and `whatjobs: 'WhatJobs'` to
...
- [ ] 3.3 Run `pnpm run check` in `web/` and confirm it is no worse than the baseline (the web
      lint baseline is red — compare, do not expect zero).

## 4. Documentation

- [ ] 4.1 Rewrite `internal/sources/AGENTS.md:12` around the crawl/taxonomy split — the
...
- [ ] 4.2 Replace the stale one-adapter claim at `internal/pipeline/AGENTS.md:11` with a
      pointer to the sources rule rather than a second copy of it.

## 5. Verify and close

- [ ] 5.1 `go build ./... && go vet ./... && go test ./...` green, including with all three
      variables unset (the default local environment).
- [ ] 5.2 Mark S7 ✅ in `docs/reviews/2026-08-01-architecture-review.md` — the shortlist row,
