...
- [ ] 1.1 Add classify test cases for `admin assistant`, `administrative
...
- [ ] 1.2 Add classify test cases proving the bare-`assistant` exclusion holds:
...
- [ ] 1.3 Add the five ADMINISTRATION aliases to
      `internal/dict/classify/dictionaries.go`; do not add a bare `assistant`
      entry
- [ ] 1.4 `go test ./internal/dict/classify/...` green

## 2. Category dictionary — legal/immigration block

- [ ] 2.1 Add classify test cases for `immigration paralegal`, `immigration
      specialist`, `immigration assistant`, `immigration consultant`,
      `immigration case manager` → `legal`
- [ ] 2.2 Add a test proving "Immigration Case Manager" resolves to `legal`
      and is not stolen by the terminal `manager` → `management` fall-through
- [ ] 2.3 Add the five immigration aliases to the LEGAL block in
      `internal/dict/classify/dictionaries.go`, ordered ahead of the bare
      `manager` fall-through
- [ ] 2.4 `go test ./internal/dict/classify/...` green

## 3. Skill dictionary — support, scheduling and legal-practice tooling

- [ ] 3.1 Add skilltag test cases: a description naming Freshdesk, Calendly,
      Clio, USCIS, "Form I-129" and "Form I-130" resolves each to its
      canonical (`freshdesk`, `calendly`, `clio`, `uscis`, `i-129`, `i-130`)
- [ ] 3.2 Add the six alias→canonical entries to
      `internal/dict/skilltag/dictionaries.go`
- [ ] 3.3 Add each canonical's reader-facing label to
      `internal/dict/skilltag/labels.go`
- [ ] 3.4 Add each canonical's one-sentence definition to
      `internal/dict/skilltag/descriptions.tsv`
- [ ] 3.5 `go test ./internal/dict/skilltag/...` green (description-coverage
      test included)

## 4. Work-mode phrases

- [ ] 4.1 Add location/workmode test cases: a description whose only remote
...
- [ ] 4.2 Add a test proving a structured `work_mode=hybrid` signal is not
      overwritten by a "telecommute" mention in the description
- [ ] 4.3 Add the six phrases to `descriptionWorkModePhrases` in
      `internal/dict/location/workmode.go`
- [ ] 4.4 Pull a sample of live descriptions containing "work from home"
      (via Meilisearch/prod query, read-only) and classify each occurrence as
      a work-arrangement statement vs. a bounded benefit/perk mention —
      decide from the sample whether "work from home" needs a
      `travelPerkPhrases` guard, mirroring how "work from anywhere" was
      decided (freehire#2696)
- [ ] 4.5 If the sample supports it, add "work from home" to
      `travelPerkPhrases` with a test proving a benefit-list sentence
      containing it does not resolve `work_mode` to `remote`
- [ ] 4.6 `go test ./internal/dict/location/...` green
...
- [ ] 5.2 `go vet ./...`
- [ ] 5.3 `go test ./...`
- [ ] 5.4 `gofmt -l .` prints nothing for changed files
