...
**Raise `ttMaxPages` to 1000, not remove it.** A page ceiling is still the right safety backstop against a board that never returns an empty page (a de-dup already guards the "same page forever" case via `newLinks==0`, but a board serving genuinely distinct junk forever is a different failure the ceiling still needs to catch) — the fix is not "no ceiling," it's "a ceiling wide enough not to be reached by a real board, paired with treating reaching it as failure rather than success." 1000 is chosen as a wide multiple over the measured real maximum (~125-129 pages for the largest board found), the same proportional margin `taleo`'s own 200-page cap and `mycareersfuture`'s 1500 already use relative to their own measured maximums.
...
- **[Trade-off]** A genuinely broken board that always returns a full page of the same junk content forever (not caught by the `newLinks==0` de-dup, e.g. a server bug that returns different junk every time) would now cost up to 1000 requests before failing, versus 100 before. → Accepted: bounded, per-board, and only on an already-broken board; not a cost any healthy board pays.
