...
`application/json` (`{"text": "..."}`), dispatched by `Content-Type`. Oversize uploads are
rejected by the server's existing global request-body limit (413). The resume file and text
SHALL NOT be persisted to disk or database, and SHALL NOT be logged; only the resulting skill
slugs are returned.
...
#### Scenario: Malformed or empty input is rejected

- **WHEN** the request contains an undecodable PDF, a non-PDF file, or empty/whitespace text
- **THEN** the system responds `400` with an `{"error": ...}` envelope and does not persist
  anything
