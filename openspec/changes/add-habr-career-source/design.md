...
(it stays in the source facet). One placeholder entry in `sources/custom.yml`.
...
- New adapter ships inside the existing server/ingest binaries — full image rebuild, **no**
  Dockerfile change.
- Relies on the existing `cmd/ingest sources/custom.yml` cron schedule (confirm it exists during
  implementation; if `custom.yml` is not yet on a schedule, add one cron line).
