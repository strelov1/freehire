...
- [ ] 1.1 In `web/src/lib/components/ExperienceBankView.svelte`, add a `confirmAtom` function that
...
- [ ] 1.2 Render a Confirm button (reuse the `Check` icon already imported) beside the existing
      ...
      same icon.
- [ ] 1.3 Confirm the copy in the "not confirmed" banner still reads correctly now that there are
...
- [ ] 2.1 In `web/src/routes/tailor/[slug]/+page.svelte`, add `'experience'` to the `LeftTab` type
...
- [ ] 2.2 Add the new panel: `<div class="h-full overflow-auto p-4" class:hidden={leftTab !==
      'experience'}><ExperienceBankView /></div>`, alongside the existing Editor/Templates/Settings
      panels — import the component.
- [ ] 2.3 Add `'experience'` to the mobile flat tab bar (`mobileTabs`) and to `pickMobile`'s
...
- [ ] 3.1 `pnpm check` in `web/` — 0 new type errors.
- [ ] 3.2 Live browser check (this repo has no Svelte component test harness): open `/tailor/[slug]`
...
- [ ] 3.3 Confirm `/my/profile`'s `ExperienceBankView` also shows the new Confirm button (shared
      component) and behaves identically there.
- [ ] 3.4 Confirm the mobile tab bar (narrow viewport) offers Experience and switching to it also
