...
- [ ] 1.1 Add migration: `jobs.content_hash text` (nullable, additive — no backfill).
- [ ] 1.2 Define the indexed-content hash in Go (next to `pipeline.Job` / the fields
...
- [ ] 1.3 Update the `UpsertJob` query: accept `content_hash`, and `RETURNING`
...
- [ ] 2.1 Thread the computed hash into `dbStore.Save`'s `UpsertJobParams` and read
  back the `inserted`/`changed` flags.
- [ ] 2.2 Decide reindex-worthiness in the store: a write needs indexing when it is
...
- [ ] 3.1 Add a batching indexer (on the `cmd/ingest` side) that buffers
...
- [ ] 3.2 Make flush errors best-effort: log and swallow; never propagate to `Save`
  or the run. Test that a failing push does not error the store/run.
- [ ] 3.3 In `dbStore.Save`, after the commit, convert reindex-worthy rows via
  `search.FromJob` and enqueue them into the indexer.

## 4. Wire the worker

- [ ] 4.1 In `cmd/ingest/main.go`, construct the search client + indexer only when
  `MEILI_URL` + master key are configured; inject into the store. Absent → nil/no-op
  indexer, ingest unchanged.
- [ ] 4.2 Call the indexer's final `Flush` after `runner.Run` (and before/independent
...
- [ ] 5.2 Integration check (testcontainers/local stack): ingest a new job and an
  edited job, confirm both appear in the live facet index without a reindex; confirm
  a no-op re-ingest pushes nothing.
- [ ] 5.3 Update `cmd/ingest` package doc and the source-ingest mention in
