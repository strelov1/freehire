...
- [x] 6.3 Add `freehire-ops' provision/host2/systemd/freehire-social-digest.service` and `.timer` (`OnCalendar=*-*-* 10:00:00 America/Sao_Paulo`, `Persistent=true`), and add `social-digest` to the binary list in `freehire-ops' scripts/host2/release.sh`. Record in the change that the host's own copy of `release.sh` must be updated by hand.
