...
- No migration, no reindex needed for this alone — `jobs.requirements_derived` is read from Postgres directly, not indexed as a search filter (per CLAUDE.md's own note on `backfill-requirements`: "Needs no reindex"). This does NOT mean existing rows are automatically refreshed: `Derive` only re-runs for a stored posting when that posting's write path executes again — a fresh ingest write, or a manual `cmd/backfill-requirements` pass. A posting already ingested before this change keeps whatever the OLD vocabulary derived for it (typically `[]`, since a Russian requirements heading previously matched nothing) until one of those two happens. `cmd/backfill-requirements`'s own `FROM_ID` cursor resumes rather than restarts, so picking up this vocabulary for postings a prior backfill pass already walked needs a run started from the beginning (`BACKFILL_REQUIREMENTS_FROM_ID` unset), not merely letting the existing cursor continue. Given the significant finding below — the one large verified real source yields zero new extractions from this vocabulary regardless — a dedicated backfill pass is not run as part of this change; it is a deliberate follow-up for whenever a `<p>`-shaped-vs-`<ul>`-shaped source where this vocabulary actually helps is confirmed.
