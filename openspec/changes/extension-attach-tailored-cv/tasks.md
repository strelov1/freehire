...
- [ ] 1.1 Add `debugger` and `downloads` permissions to `wxt.config.ts`'s manifest block.

## 2. Tailored-CV resolution (`lib/freehire.ts`)

- [ ] 2.1 Add a pure function that picks the matching tailored CV (if any) for a job slug
...
- [ ] 2.2 Add `getTailoredCVForJob(slug, token)` wiring that calls `GET /me/cvs` and applies
      2.1's picker.
- [ ] 2.3 Add `getCVPdfBytes(id, token)` (or reuse an existing fetch helper) to pull the
      rendered PDF from `GET /me/cvs/:id/pdf` as bytes.

## 3. Frame-scoped element resolution

- [ ] 3.1 Add a pure function that, given `extractUploads`'s reported `{form, frame}` for the
...
- [ ] 4.1 Add a background helper that base64-encodes PDF bytes into a `data:` URL, calls
...
- [ ] 4.2 Test the promise/timeout/error-classification logic with a faked
...
- [ ] 5.1 Add a background handler that, given a tab id, a resolved file path (from §4) and
...
- [ ] 5.2 Classify and surface the two named failure modes distinctly: tab already open in
      ...
      design.md.
- [ ] 5.3 Wire the panel-to-background message (extending `protocol.ts`'s `RuntimeMessage`)
