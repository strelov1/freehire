...
- **Downstream, for free:** `internal/candidate/matchanalysis` and `internal/candidate/coverletter` already read `enrichment.requirements`, so they inherit the wider coverage without a change.
