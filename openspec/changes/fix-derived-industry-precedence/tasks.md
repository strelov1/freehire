...
- [ ] 1.1 Postgres: guard the derived arm of the `industries` predicate with
      `cardinality(industries) = 0` in `ListCompanies` and `CountCompanies`, then
      `make sqlc`
- [ ] 1.2 Meilisearch: make the derived fragment a conjunct with `industries IS EMPTY`
      in `CompanyFilterFromValues`, keeping the group a plain OR of the two arms
- [ ] 1.3 Integration tests: a company whose curated industry does not match is absent
...
- [ ] 2.1 Map `media` → `entertainment` and `mobility` → `transportation`, leaving
...
- [ ] 2.2 Add the missing aliases: `media`, `publishing`, `digital-publishing`,
      `social-media`, `creator-economy` → `entertainment`; `mobility`,
      `urban-mobility`, `ride-hailing`, `mobility-as-a-service` → `transportation`
- [ ] 2.3 Update the dictionary tests: the deliberate-omission list is now `other`
      alone, and the two new pairs resolve

## 3. Verify against the reported defect

- [ ] 3.1 A test reproducing the production case: a company shaped like Uber (curated
