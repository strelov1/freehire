...
- [ ] 1.1 Extract an exported effective-posting-date helper in `internal/jobview`
...
- [ ] 2.1 Add a derived `posted_ts` (unix seconds of the effective posting date)
...
- [ ] 4.1 Extend the search filter builder (`internal/search/query_filter.go`) so
...
- [ ] 5.1 Add `postedWithinDays: number | null` to `JobFilters`, round-trip it in
  `filtersToParams` / `filtersFromParams` as `posted_within_days`, and add a
  `setPostedWithinDays(n)` store method that applies immediately (`setNow`).
  Verify the round-trip (params ↔ state) for set and cleared values.

## 6. Frontend freshness slider

- [ ] 6.1 Add the preset list `[Today=1, 3d, week=7, 2 weeks=14, month=30,
  3 months=90, Any=null]` near `SALARY_MAX` in `web/src/lib/facets.ts`.
- [ ] 6.2 Render a discrete-preset range slider at the **top** of
...
- [ ] 7.1 Run `go build ./... && go vet ./... && go test ./...` and the web
  checks; confirm the new param filters search results.
- [ ] 7.2 Record the post-deploy `cmd/reindex` requirement (so `posted_ts`
  populates existing jobs) in the change's deploy notes.
