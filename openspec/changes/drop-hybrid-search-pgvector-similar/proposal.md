...
- **Changed**: `GET /api/v1/me/recommendations` (CV-based) queries `job_semantic_chunks` directly via pgvector (same nearest-chunk rollup as `/similar`, combined with a `WHERE` clause built from the same facet-filter translation it uses today) instead of Meili's hybrid search. This stays a live, per-request query — it is the one legitimate remaining live use of vector search, since the query vector (a specific user's CV) and the facet filters are both only known at request time.
