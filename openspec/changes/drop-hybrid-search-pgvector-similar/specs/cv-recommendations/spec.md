...
**Reason**: `/me/recommendations`, the only consumer of the persisted CV embedding, is removed in this same change (see below) — migrating it to pgvector would have needed either a new SQL translation of the ~20+-facet Meili filter semantics (large, correctness-risky, duplicating already-trusted logic) or a capped two-stage Meili-then-pgvector lookup; the user judged that complexity not worth it for a feature of uncertain usage and chose removal over migration. With no reader left, computing and storing the vector on every CV upload is pure waste.

**Migration**: No user-facing migration — this was never user-visible (a derived vector, not shown or exported anywhere). The write path (on résumé upload) is deleted alongside the read path. Already-persisted CV vectors are inert data; cleaning up the storage column, if any, is a separate concern from this capability's removal and not addressed here (matches this change's general policy of not bundling column drops into the change that stops using them).
...
**Reason**: Depends on `jobs_semantic` (removed in this same change) and a live per-request vector query combined with arbitrary facet filters. Migrating it needs real new complexity (see the removed embedding-persistence requirement's Reason). The user chose to drop the feature rather than carry that complexity forward, for a feature of uncertain usage.

**Migration**: `GET /api/v1/me/recommendations` is removed (404 for any caller — no deprecation window, per the user's explicit choice to remove outright). The route, handler, and its tests are deleted.
...
**Reason**: The endpoint this sort mode depends on is removed (see above).

**Migration**: The sort control is removed from the standalone feed entirely — no "Recommended" option, no `sort=cv` handling. A pre-existing shared link carrying `?sort=cv` simply falls back to the default "Newest" behavior (the param is no longer recognized, matching this endpoint's existing "unknown params are ignored" convention) rather than erroring.
