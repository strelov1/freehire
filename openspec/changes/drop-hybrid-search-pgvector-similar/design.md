...
LIMIT $2
```
(Exact predicate/index usage to be validated during implementation — this is a heavier query shape than a plain `ORDER BY embedding <=> $1`, a chunk-to-chunk cross join before the per-job rollup; see Risks. The same-company exclusion is a plain `company_slug` comparison, not a vector operation, and composes trivially with the rest.) This same "nearest chunk, rolled up per job" shape is reused by `/recommendations`' live query (task 6.x) — a CV is not chunked (a résumé is short enough to embed as one passage), so that side of the join is always a single vector, but the rollup-per-job logic is identical.
...
- **[Trade-off] `/recommendations` becomes MORE correct but also changes results** for any user whose recommendations previously excluded a job purely because `jobs_semantic`'s `is_tech` gate dropped it, AND now additionally changes because the underlying embeddings themselves are richer (full description, not the first 15-20%). Both are desired effects of the fix, not regressions, but worth calling out as a visible behavior change, not a pure refactor.
- **[Risk] The chunk-join nearest-neighbour query (Decision 5) is a genuinely heavier query shape than the single-vector case the original spike measured.** Not yet benchmarked at any scale this session. → **Mitigation**: this query only ever runs inside `cmd/similar-backfill` (a background batch job, not the live `/similar` request path — Goal 1 is unchanged, the precomputed-lookup property holds regardless of how expensive computing it is) and inside the low-QPS authenticated `/recommendations` path — neither is latency-critical the way the originally-considered "hybrid search on every keyword query" idea was. Still: measure it for real during implementation rather than assuming it scales fine.
...
7. Switch `/me/recommendations` to the live pgvector query (old Meili-backed path removed only now).
