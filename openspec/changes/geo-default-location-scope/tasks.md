...
- [x] 5.3 Measured with the country pinned by header, three alternating pairs, real prod data. **CLS 0.0014 → 0.026** (Google's "good" bar is ≤0.10). **LCP 888-1228ms baseline vs 984-1408ms scoped** — overlapping ranges, no signal above dev-server noise. Before the two fixes above the same measurement read 0.87
- [x] 5.4 **Decision: ship the automatic form.** The gate asked for no regression in the layout-shift figure; 0.026 sits inside "good" with room to spare, and LCP is unmoved. The suggestion form stays recorded in design.md as the fallback if field data disagrees with the lab
