...
- [ ] 1.1 Replace `searchintent`'s `role` enum with `category` + `seniority`, built from the same vocabularies the rest of the request uses
- [ ] 1.2 Update the interpreter's tests, including the contradiction cases — a grade and a specialization are now two fields, so a proposal can disagree with itself in a way one slug could not
