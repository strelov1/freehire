...
- [ ] 1.1 Create `web/src/lib/docs/api-spec.ts`: the typed model (`Param`,
...
- [ ] 1.2 Create `web/src/lib/docs/filters.ts`: build the job-search filter table
...
- [ ] 2.1 Create `web/scripts/gen-api-docs.mjs`: import the spec data via
  vite-node, render `docs/API.md` (generated-header + all groups/endpoints/filter
  table), write to repo root `docs/API.md`.
- [ ] 2.2 Add `"gen:api-docs"` script to `web/package.json` and a
...
- [ ] 2.3 Run the smoke check; confirm `docs/API.md` is produced with the
  generated header and is stable across runs.

## 3. Page and component

- [ ] 3.1 Create `web/src/lib/components/DocsApiView.svelte`: sticky table of
...
- [ ] 3.2 Create `web/src/routes/docs/api/+page.svelte` (renders `DocsApiView`)
  and `+page.ts` (title + meta description; ensure SSR, no auth).

## 4. Navigation and cross-links

- [ ] 4.1 Add an "API" link to `web/src/lib/components/TopBar.svelte` (desktop +
  mobile nav) pointing to `/docs/api`.
- [ ] 4.2 Cross-link the docs page from `CliView.svelte` and `ApiKeysView.svelte`
  as the full API reference.

## 5. Generate and verify

- [ ] 5.1 Run `npm run gen:api-docs`; commit the generated `docs/API.md`.
- [ ] 5.2 Run `npm run check` (svelte-check) and `npm run lint` on the new files;
  confirm no new errors versus the baseline.
- [ ] 5.3 Visually verify `/docs/api` renders server-side in dev and that the
