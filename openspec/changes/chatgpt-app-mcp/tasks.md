...
- [ ] 1.1 Register `internal/api/mcpapp` in `internal/platform/arch/layering/blocks.go` (the
...
- [ ] 1.2 Create the package with its `Reader` interface — the four tool answers, already
      projected — and nothing else. Prove the interface is satisfiable by a test fake before
      any handler implements it.

## 2. Wire shapes

- [ ] 2.1 Define the search result shape the tools answer in: our fields, the index's
...
- [ ] 2.2 Define the job-detail shape: the same fields plus the full description and the
      stored apply form's required fields when one exists.
- [ ] 2.3 Define the company shapes for `search_companies` and `get_company`, each carrying
      the company's freehire page URL and its open-posting count.
- [ ] 2.4 Define the search input: the curated ~15 parameters named in the spec, each with a
...
- [ ] 3.1 `search_jobs`: map the input onto `url.Values` and hand it to
...
- [ ] 3.2 Carry unreadable parameters through into the result's ignored list. Test that an
      unknown filter widens and is named, rather than narrowing.
- [ ] 3.3 Enforce the page maximum, and report the total separately from the returned count.
- [ ] 3.4 `get_job`: one posting with its full body by public slug; an unknown slug yields
      the recoverable error result.
- [ ] 3.5 `search_companies` and `get_company` against the existing company reads.
- [ ] 3.6 Assert the visibility predicate is the shared one: a private posting, a
...
- [ ] 4.1 Build the `mcp.Server` with a server name distinct from `freehire-ojcp` and
      instructions under 512 characters. Assert the length in a test — the limit is a
      number nobody re-checks by eye.
- [ ] 4.2 Register the four tools with title, description, input schema, output schema and
...
- [ ] 4.3 Render failures as `CallToolResult` with `IsError` and a plain sentence, never as
      `*jsonrpc.Error`. Test both the absent case and the internal-failure case, and assert
      the second discloses no internal detail.
- [ ] 4.4 Drive the whole server through the SDK's in-memory client in tests — `initialize`,
      `tools/list`, and one call per tool — the way `ojcpmcp_test.go` does.

## 5. Route

- [ ] 5.1 Add `internal/api/handler/mcpapp.go`: implement `mcpapp.Reader` on the existing
      search handlers and mount `api.All("/mcp", limit, adaptor.HTTPHandler(...))` with the
      same `agentSearchLimiter` the OJCP routes use.
- [ ] 5.2 Integration test: the route answers `initialize` over HTTP, and the OJCP route
      still answers with its own server name and error shape.
- [ ] 5.3 `go vet -tags=integration ./...` and `go test -tags=integration ./internal/api/...`
...
- [ ] 6.2 Document the surface in `internal/api/mcpapp/AGENTS.md`: why a second MCP server
