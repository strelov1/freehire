...
- [x] 1.1 Write `migrations/0071_jobs_closed_reason.sql`: add `closed_reason text NOT NULL
...
- [x] 1.2 Add a failing integration test (`internal/db`, tag `integration`) asserting that
...
- [x] 1.3 Set the reason in those five queries in `internal/db/queries/jobs.sql`, run
  `make sqlc`, update the Go callers for any changed signatures. Tests green.
- [x] 1.4 Add a failing test that a reopened job carries no reason, then clear
  `closed_reason` alongside `closed_at = NULL` in `UpsertJob`, `UpsertManualJob` and
  `TouchJob`. `make sqlc`. Tests green.

## 2. The age rule

- [x] 2.1 Add a failing integration test for the new query: a `telegram` row 46 days old
...
- [x] 2.2 Write `CloseStaleUnsignalledJobs` in `internal/db/queries/jobs.sql` — closes open
...
- [x] 2.3 Call it from `cmd/liveness` under the existing advisory lock, passing
...
- [x] 3.1 Update `docs/agents/job-lifecycle.md`: add the age rule as mechanism (4) to the
...
- [x] 3.2 Update `internal/telegram/AGENTS.md`, whose Limitations section currently states
  that Telegram jobs stay open until something else closes them.

## 4. Verification

- [x] 4.1 `go test ./...` and `go test -tags=integration ./internal/db/`. Both green.
- [x] 4.2 Dry-run the count against prod before the first live run: the age rule should match
