...
- [ ] 1.1 Implement the `icims` `Source` adapter in `internal/sources/icims.go`
...
- [ ] 1.2 Register `NewICIMS(c)` in `sources.All` (`internal/sources/source.go`).

## 2. Harvest prober

- [ ] 2.1 Widen the `cmd/harvest-boards` prober `httpClient` interface with
...
- [ ] 2.2 Register `"icims": icimsProber{}` in the `probers` map
  (`cmd/harvest-boards/prober.go`).

## 3. Verify

- [ ] 3.1 `go build ./... && go vet ./... && go test ./...` all green.
