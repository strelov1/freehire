...
- [ ] 1.1 Add a failing test in `internal/sources/workday_test.go`: page 1 reports
...
- [ ] 1.2 Add `workdayCapTotal = 2000` and a `facets`/`appliedFacets` decode to the
      page-response struct in `listPostings`.
- [ ] 1.3 Implement the single-level split: on `total == workdayCapTotal`, pick the
...
- [ ] 1.4 Run `go test ./internal/sources/... -run Workday` — new test green,
...
- [ ] 2.1 Add a failing test: a facet value's own slice still reports `total: 2000`
...
- [ ] 2.2 Add a failing test: recursion stops at `maxFacetDepth = 3` combined
      dimensions — a slice still capped at that depth is paged as-is (best effort),
      not recursed further.
- [ ] 2.3 Implement `maxFacetDepth` and the "not-yet-used dimension" selection,
      threading which dimensions are already applied down each recursion branch.
- [ ] 2.4 Run `go test ./internal/sources/... -run Workday`.

## 3. Dedup across slices

- [ ] 3.1 Add a failing test: two recursed slices both return a posting with the
      same `externalPath` (simulating a multi-counting dimension like
      `workerSubType`); the final result contains it once.
- [ ] 3.2 Implement dedup by `ExternalPath` when merging slice results in
      `listPostings`.
- [ ] 3.3 Run `go test ./internal/sources/... -run Workday`.

## 4. Depth-exhaustion and no-dimension logging

- [ ] 4.1 Add a failing test (capturing `log` output or via a seam already used
...
- [ ] 4.2 Implement the log line(s), matching the `provider: message` convention
      already used in `config.go` / `eightfold.go` / `djinni.go`.
- [ ] 4.3 Run `go test ./internal/sources/... -run Workday`.

## 5. Boards below the cap are unaffected

- [ ] 5.1 Add/confirm a test: `total < workdayCapTotal` on page 1 pages exactly as
...
- [ ] 5.2 Run the full `internal/sources` suite to confirm no regression to any
      other provider or to Workday's non-capped path.

## 6. 403 retry

- [ ] 6.1 Add a failing test: a `PostJSON`/`GetJSON` call returns a 403 once, then
      succeeds; the Workday adapter retries and returns the successful result
      instead of failing the board.
- [ ] 6.2 Add a failing test: a non-retryable error (e.g. 404) returns immediately
      without retrying.
- [ ] 6.3 Implement a Workday-local retry wrapper mirroring `eightfold.go`'s
...
- [ ] 6.4 Run `go test ./internal/sources/... -run Workday`.
