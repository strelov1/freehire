...
## 8. Second independent review pass

- [x] 8.1 `RefreshCompanyFacets` (`internal/platform/db/queries/companies.sql`) hand-duplicates `search.CategoryUnresolved`'s old logic in raw SQL and was not updated with the `is_tech` carve-out — a company whose only open jobs are confirmed-technical-but-uncategorized would show 0 open jobs on its page/sitemap while those same jobs are live in search. Added the `is_tech IS TRUE` clause and a regression test (`TestRefreshCompanyFacetsCountsConfirmedTechnicalJobs`).
- [x] 8.2 `internal/ingest/linkimport.go`'s `draftFrom` — a second `sources.Job → jobderive.Input` mapping site, for pasted-link import — did not forward `IsTechHint`, reproducing issue #2601 through that entry point. Wired it through, with a regression test.
- [x] 8.3 Added a focused unit test (`TestUpsertParams_IsTechHintMovesIsTechIndependentlyOfContentHash`) proving `IsTechHint` moves `is_tech` without moving `content_hash`, since the generic per-field match-key test's shared fixture (`Category: "backend"`) already saturates tech evidence and cannot exercise this on its own.
- [x] 8.4 Documented the actual rollback procedure in design.md (a full `make reindex` is needed to remove jobs the widened rule already admitted; the code/backfill alone are harmless to revert) — the file previously understated this as "No rollback complexity."
- Considered and declined: an `itops`-board `Fetch`-level regression test in `profession_test.go` (redundant with `TestProfessionITBoardNames`/`TestProfessionConfirmsTech`, which already assert both board names); switching `BackfillProfessionITBoardTech`'s `ILIKE ANY` to a case-sensitive form for index-friendliness (low severity, one-off bounded to ~2 boards); unifying `sources.ProfessionConfirmsTech`'s `strings.Cut(externalID, ":")` with `applyform.splitBoardPosting`'s identical one-liner behind a shared `externalid.Split` (real but trivial duplication, left as a follow-up).
