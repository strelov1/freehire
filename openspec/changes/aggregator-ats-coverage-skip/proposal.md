...
- Add an ingest-time gate: before saving a posting from an aggregator-classified provider,
  skip the write if the company (folded `company_slug`) already has an open posting from a
...
- A new optional `Runner.Coverage` port in `internal/pipeline`, following the existing
  ...
  test fakes, an environment without `MEILI_URL` configured).
- A new `Stats.ATSCovered` counter and per-board log line, kept separate from the existing
  `Rejected` (non-technical) counter.
- `cmd/ingest` gains a new optional dependency on `MEILI_URL`, needed only for
  aggregator-provider board files.
...
- `cmd/ingest/main.go`: wire the adapter when `MEILI_URL` is set.
- Ops: `MEILI_URL` needs adding to the aggregator-provider ingest systemd units (out of scope
  for the code change itself, called out for deploy).
